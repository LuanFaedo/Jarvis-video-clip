...
# --- NOVAS DEPENDÊNCIAS DE ÁUDIO (V64) ---
try:
    import librosa
    import numpy as np
    ...
    print("[AVISO] Librosa não instalado. Análise musical profunda desativada.")
...
def limpar_processos_orfãos():
    """Mata processos Node e Chrome persistentes para evitar travamento de sessão."""
    print("[SISTEMA] Limpando processos órfãos...", flush=True)
    try:
        if sys.platform.startswith("win"):
            pass
            # os.system("taskkill /F /IM node.exe /T >nul 2>&1")
            # Cuidado com chrome.exe para não fechar o navegador do usuário, 
            # mas aqui o foco é o servidor dedicado.
...
from video_director import iniciar_processo_video
...
def carregar_memoria_md():
    """Carrega memory.md com tratamento robusto de encoding (UTF-8 com fallback)."""
    path = os.path.join(BASE_DIR, "memory.md")
    ...
    try:
        ...
        print("[SISTEMA] Aviso: memory.md contém caracteres inválidos para UTF-8. Tentando fallback...")
        try:
            with open(path, "r", encoding="cp1252", errors="ignore") as f:
                return f.read()
        except Exception as e:
            print(f"[SISTEMA] Falha no fallback de leitura de memória: {e}")
            return ""
    except Exception as e:
        print(f"[SISTEMA] Erro ao ler memory.md: {e}")
        return ""
...
def gerar_resposta_jarvis(user_id, texto):
    ...
    # --- VIDEO DIRECTOR (V27) ---
    gatilhos_video = [
        "criar video", "fazer video", "gera video", "gerar video", "cria video", "faz um video",
        "criar clipe", "fazer clipe", "videoclipe", "video de produto", "comercial sobre",
        "video comercial", "video de venda", "propaganda de", "fazer um video"
    ]
    texto_limpo = texto.lower()
    if any(kw in texto_limpo for kw in gatilhos_video) or (("video" in texto_limpo or "vídeo" in texto_limpo) and ("cria" in texto_limpo or "faz" in texto_limpo or "gera" in texto_limpo)):
        dados, resposta_imediata = iniciar_processo_video(user_id, texto)
        if dados: # Se o director aceitou o job
            adicionar_mensagem(user_id, "user", texto)
            adicionar_mensagem(user_id, "assistant", resposta_imediata)
            return {"text": resposta_imediata, "pdf": None}
...
=== CRIAÇÃO DE VÍDEO (V27) ===
- Você PODE criar videoclipes e comerciais para produtos (usando links).
- Se o usuário pedir um vídeo, responda de forma afirmativa para ativar o gatilho (Ex: "Entendido, iniciando a criação do seu vídeo..."). O sistema Python cuidará do resto.
- Palavras-chave para ativar: "criar video", "fazer clipe", "video comercial".
...
async def _tts_async(text, path):
    try:
        with open(CONFIG_FILE, 'r', encoding='utf-8') as f: speed = json.load(f).get('velocidade', '+0%')
    except: speed = '+0%'
...
def analisar_caracteristicas_audio(file_path):
    """Extrai BPM e Energia de músicas usando Librosa (Ouvido Absoluto)."""
    if not LIBROSA_AVAILABLE: return "Análise indisponível (Librosa missing)."
    
    try:
        print(f"[AUDIO] Analisando espectro de: {os.path.basename(file_path)}...", flush=True)
        # Carrega apenas os primeiros 60s para performance
        y, sr_rate = librosa.load(file_path, duration=60)
        
        # 1. Detectar BPM (Tempo)
        onset_env = librosa.onset.onset_strength(y=y, sr=sr_rate)
        tempo, _ = librosa.beat.beat_track(onset_envelope=onset_env, sr=sr_rate)
        bpm = float(tempo)
        
        # 2. Detectar Energia (RMS)
        rms = librosa.feature.rms(y=y)
        energia_media = np.mean(rms)
        
        # Classificação
        vibe = "Moderada"
        if bpm > 120 and energia_media > 0.1: vibe = "Agitada/Alta Energia"
        elif bpm < 90 and energia_media < 0.05: vibe = "Calma/Melancólica"
        elif energia_media > 0.2: vibe = "Intensa/Barulhenta"
        
        relatorio = f"BPM: {bpm:.1f} | Energia: {energia_media:.3f} | Vibe: {vibe}"
        print(f"[AUDIO RESULTS] {relatorio}")
        return relatorio
    except Exception as e:
        print(f"[ERRO LIBROSA] {e}")
        return "Erro na análise musical."

def transcrever_audio(base64_data, extension=".ogg"):
    if not base64_data: 
        print("[AUDIO] Erro: Dados base64 vazios.")
        return None, None # Retorna Tupla (Texto, InfoMusical)
    ...
    try:
        ...
        ext_final = extension if extension.startswith(".") else f".{extension}"
        
        with tempfile.NamedTemporaryFile(suffix=ext_final, delete=False) as t:
            t.write(audio_bytes)
            temp_p = t.name
        
        # --- ANÁLISE PROFUNDA (Librosa) ANTES DA CONVERSÃO ---
        # Se for um arquivo de música (mp3, ogg, wav), analisamos a "vibe"
        if LIBROSA_AVAILABLE and ext_final in ['.mp3', '.wav', '.ogg', '.m4a']:
            info_musical = analisar_caracteristicas_audio(temp_p)

        # Converter para WAV 16kHz Mono (Padrão ouro para STT)
        temp_wav = temp_p + ".wav"
        
        print(f"[AUDIO] Pré-processando com Pydub (Normalização)...", flush=True)
        try:
            # Pydub: Carrega e Normaliza (Aumenta volume se baixo)
            audio = AudioSegment.from_file(temp_p)
            
            # Normalização simples: Aumentar para -20dBFS se estiver muito baixo
            if audio.dBFS < -20:
            ...
            print(f"[AUDIO] Pydub falhou ({conv_err}). Tentando FFMPEG direto...", flush=True)
            try:
                subprocess.run([
                    "ffmpeg", "-y", "-i", temp_p, 
                    "-ar", "16000", "-ac", "1", 
                ...
                return "[ERRO AUDIO FORMATO]", info_musical

        # --- TENTATIVA 1: VOSK (OFFLINE) ---
        texto = ""
        print("[AUDIO] Reconhecendo fala (OFFLINE VOSK)...", flush=True)
        texto_vosk = audio_scanner_offline.transcrever_audio(temp_wav)
        
        if "ERRO" not in texto_vosk and len(texto_vosk) > 2:
            texto = texto_vosk
        else:
            print(f"[AUDIO] VOSK falhou ou vazio. Tentando Google...", flush=True)
        ...
        print(f"[AUDIO] Transcrição Final: '{texto}' | Info: {info_musical}")
        return (texto if texto.strip() else None), info_musical

    except Exception as e:
        print(f"[AUDIO] Erro geral na transcrição: {e}")
        return None, None
    finally:
        try:
...
def carregar_audio_universal(caminho_arquivo):
    print(f"🎵 [AUDIO] Tentando carregar: {caminho_arquivo}")
    
    try:
        # Tentativa 1: Leitura Direta (Rápida)
        return AudioFileClip(caminho_arquivo)
    except Exception as e:
        print(f"⚠️ [AUDIO] Leitura direta falhou ({e}). Iniciando conversão forçada...")
        
        # Tentativa 2: Conversão para WAV via FFmpeg (Blindada)
        arquivo_temp = f"{caminho_arquivo}.converted.wav"
        if os.path.exists(arquivo_temp): os.remove(arquivo_temp)
        
        # Comando FFmpeg para converter qualquer coisa para WAV simples
        comando = [
        ...
        if os.path.exists(arquivo_temp):
            return AudioFileClip(arquivo_temp)
        else:
            print("[AUDIO] Falha catastrófica na conversão de áudio.")
            return None
...
def api_whatsapp():
    try:
        ...
        if data.get('audio_data'):
            # --- UNIVERSAL EXTENSION MAPPER ---
            mime = data.get('mimetype', '').lower()
            ext = ".ogg" # Default safe fallback
            
            # Aceitar QUALQUER coisa que pareça áudio ou vídeo
            if "mp3" in mime: ext = ".mp3"
            elif "wav" in mime: ext = ".wav"
            elif "ogg" in mime: ext = ".ogg"
            elif "opus" in mime: ext = ".opus"
            elif "aac" in mime: ext = ".aac"
            elif "m4a" in mime: ext = ".m4a"
            elif "flac" in mime: ext = ".flac"
            elif "wma" in mime: ext = ".wma"
            elif "mp4" in mime: ext = ".mp4"
            elif "mov" in mime: ext = ".mov"
            elif "webm" in mime: ext = ".webm"
            elif "video" in mime: ext = ".mp4" # Fallback genérico para vídeo
            elif "audio" in mime: ext = ".mp3" # Fallback genérico para áudio

            # Salvar áudio bruto para uso no pipeline
            try:
                raw_bytes = base64.b64decode(data['audio_data'])
                raw_audio_path = os.path.join("audios", f"upload_{int(time.time())}{ext}")
                with open(raw_audio_path, "wb") as f: f.write(raw_bytes)
            except Exception as e: print(f"[AUDIO SAVE ERRO] {e}")

            # Passa a extensão correta para o transcritor (STT)
            # AGORA RECEBE TUPLA: (Texto, InfoMusical)
            trans, info_musical = transcrever_audio(data['audio_data'], extension=ext)
            
            if trans: 
                texto = f"{texto} {trans}".strip()
            
            # Se houver info musical (BPM/Vibe), adiciona ao contexto
            if info_musical:
                texto += f"\n[ANÁLISE DE ÁUDIO/MÚSICA]: {info_musical}"
            
            if not trans and not info_musical and not texto.strip(): 
                 texto = "SISTEMA: Áudio recebido mas não entendido (Ruído ou Música Instrumental)."

        # --- TRIGGER PIPELINE DE VIDEO (V64 - UNIVERSAL LOADER) ---
        audio_duration = 0
        is_long_audio = False
        
        if raw_audio_path:
            try:
                # Usa o novo carregador universal (FFmpeg fallback)
                audio_clip = carregar_audio_universal(raw_audio_path)
                if audio_clip:
                    audio_duration = audio_clip.duration
                    audio_clip.close() # Libera o arquivo
                    print(f"[AUDIO ANALYSER] Duração: {audio_duration:.1f}s", flush=True)
                    
                    if audio_duration > 50:
                        is_long_audio = True
            except Exception as e:
                print(f"[AUDIO DURATION ERRO] {e}")

        texto_lower = texto.lower()
        keywords = ["fazer clipe", "criar clipe", "videoclipe", "fazer vídeo", "fazer video", "criar video", "fazer clip", "criar clip"]
        
        # GATILHO: Palavra-chave OU Áudio Longo (>50s) OU Detecção Musical (BPM/Energia)
        is_musical_vibe = "BPM" in texto # Se a análise musical detectou algo
        
        if raw_audio_path and (any(k in texto_lower for k in keywords) or is_long_audio or is_musical_vibe):
            print(f"[PIPELINE V70] Trigger Ativado! (Duration: {audio_duration}s)", flush=True)
            
            trigger_reason = "Detecção de Música (Análise Espectral)" if is_musical_vibe else "Comando de Usuário"
            if is_long_audio: trigger_reason = "Áudio Longo (>50s)"
            
            # O novo music_video_handler gerencia o roteiro, análise de letra e blocos sequenciais.
            # Ele também cuida da notificação PUSH via JSON para o WhatsApp.
            tema_final = texto
            if "BPM:" in texto and len(texto) < 200:
                 tema_final = f"Vibração musical: {texto}"

            # Inicia a thread avançada V70 (Navegador visível e continuidade total)
            threading.Thread(target=_thread_processar_video, args=(sender, raw_audio_path, tema_final)).start()
            
            msg_resposta = f"🎶 {trigger_reason}: Iniciando a produção avançada (V70) do videoclipe ({int(audio_duration)}s). O navegador Brave será aberto para você acompanhar a criação!"
            return jsonify({"response": msg_resposta, "chat_id": chat_id})
